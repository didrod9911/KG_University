...
<meta charset="UTF-8">
<title>동아리게시판 - ${cbContent.content}</title>
	...
	<div id="div_root">
		...
		<div id="section1">
			...
			<table border="0" width="90%">
				<tr>
					...
					<td style="text-align: right;">
						<input type="button" value="목록" onclick="location.href='ClubBoard_List.do'"
								class="btn btn-primary" style="font-size: 18px; font-weight:bold;">
					</td>
					<td width="90" style="text-align: right;">
						<input type="button" value="글쓰기" onclick="location.href='ClubBoard_Write.do'"
								class="btn btn-primary" style="font-size: 18px; font-weight:bold;">
					</td>
			...
			<table border="0" width="90%">
				<tr>
					<td colspan="5" style="text-align: left;">
						<font size="4" style="font-weight: bold;">
							<c:choose>
								<c:when test="${cbContent.type eq 'gongji'}">
									[공지] ${cbContent.title}
								</c:when>
								<c:when test="${cbContent.type eq 'hongbo'}">
									[홍보] ${cbContent.title}
								</c:when>
								<c:when test="${cbContent.type eq 'hang'}">
									[행사] ${cbContent.title}
								</c:when>
								<c:otherwise>
									[일반] ${cbContent.title}
				...
				<tr style="text-align: left;">
					<td><font style="font-weight: bold;">글번호</font> ${cbContent.CBN_seq}</td>
					<td width="400" style="text-align: right; padding-right: 20px;">
						<font style="font-weight: bold;">등록일</font> ${cbContent.regDate}
						<c:if test="${not empty cbContent.modifyDate}">
							&nbsp;&nbsp;<font style="font-weight: bold;">수정일</font> ${cbContent.modifyDate}
						</c:if>
					</td>
					<td width="80"><font style="font-weight: bold;">조회수</font> ${cbContent.hit}</td>
					<td width="150"><font style="font-weight: bold;">글쓴이</font> ${cbContent.userNickname}</td>
					<td width="80" style="text-align: right;"><font style="font-weight: bold;">댓글</font> ${cbContent.commentCnt}개</td>
				</tr>
			</table>
			<hr width="90%" style="border: solid 1px;">
			
			<div style="width: 90%; text-align: left; padding-left: 5%;">
				<c:if test="${not empty cbContent.imgName}">
					<img src ="<spring:url value='/resources/boardImg/${cbContent.imgName}'/>"><br>
					<br><br>
				</c:if>
				<pre><c:out value="${cbContent.content}" /></pre>
			</div>
			<br><br>
			
			<table width="90%">
				<tr><td style="text-align: left; font-weight: bold;">댓글 ${cbContent.commentCnt}개</td></tr>
			</table>
			<hr width="90%" style="border: solid 1px;">
			...
			<table class="table table-hover">
				<c:forEach var="cbComment" items="${cbComment}" begin="${(boardCommentPagingVO.pageNum-1)*boardCommentPagingVO.perPageCnt}"
																end="${boardCommentPagingVO.pageNum*boardCommentPagingVO.perPageCnt-1}">
					<tr>
						<td width="150" height="30" style="text-align: left;"><c:out value="${cbComment.userNickname}" /></td>
						<td style="text-align: left;"><pre><c:out value="${cbComment.boardComment}" /></pre></td>
						<td width="200" style="text-align: right;"><c:out value="${cbComment.regDate}" /></td>
						
						<td width="10" style="text-align: right;">
							<%-- <button class="badge badge-secondary" style="font-size: 11pt; padding: 3px;"
							onclick="commentUpdateCheck('${cbComment.boardComment}', '${cbComment.CBC_seq}', '${cbComment.userID}', '${cbComment.commentPW}')">수정</button> --%>
							<button id="${'comment'+= cbComment.CBC_seq}" value="${cbComment.boardComment}"
							class="badge badge-secondary" style="font-size: 11pt; padding: 3px;"
							onclick="commentUpdateCheck('${cbComment.CBC_seq}', '${cbComment.userID}', '${cbComment.commentPW}')">수정</button>
						</td>
						
						<td width="10" style="text-align: right;">
							<button type="button" class="badge badge-danger" style="font-size: 11pt; padding: 3px 6px;"
							onclick="commentDelete('${cbComment.CBC_seq}', '${cbComment.userID}', '${cbComment.commentPW}')">X</button>
						</td>
					</tr>
				</c:forEach>
			</table>
			...
			<c:if test="${boardCommentPagingVO.pageNum ne 1}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="ClubBoard_Content.do?cPage=1&cNum=${cbContent.CBN_seq}">처음</a>
				</font>
			</c:if>
			<c:if test="${boardCommentPagingVO.prev}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="ClubBoard_Content.do?cPage=${(boardCommentPagingVO.pageBlock-2) * boardCommentPagingVO.displayPageNum + 1}&cNum=${cbContent.CBN_seq}">이전</a>
				</font>
			</c:if>
			&nbsp;
			<c:forEach var="cPage" begin="${boardCommentPagingVO.startPage}" end="${boardCommentPagingVO.endPage}">
				<c:choose>
					<c:when test="${boardCommentPagingVO.pageNum eq cPage}">
						<font style="font-size: 13pt; font-weight: bold;">
							<a href="ClubBoard_Content.do?cPage=${cPage}&cNum=${cbContent.CBN_seq}">${cPage}</a>
						</font>
					</c:when>
					<c:otherwise>
			...
			<c:if test="${boardCommentPagingVO.next}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="ClubBoard_Content.do?cPage=${boardCommentPagingVO.pageBlock * boardCommentPagingVO.displayPageNum + 1}&cNum=${cbContent.CBN_seq}">다음</a>
				</font>
			</c:if>
			<c:if test="${boardCommentPagingVO.pageNum < boardCommentPagingVO.lastPage}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="ClubBoard_Content.do?cPage=${boardCommentPagingVO.lastPage}&cNum=${cbContent.CBN_seq}">마지막</a>
				</font>
			</c:if>
			<!-- 댓글페이징부-end -->
			...
			<hr width="90%" style="border: solid 1px;">
			<br>
			<form action="ClubBoard_Content_CommentUpload.do">
			<input type="hidden" name="cPage" value="${boardCommentPagingVO.lastPage}">
			<input type="hidden" name="cNum" value="${cbContent.CBN_seq}">
<!-- 세션확인, 닉네임도필요 -->
			...
			<table border="0" width="90%">
				<tr>
					<td height="40" style="text-align: left;">
						<input type="password" name="cbCommentPW" class="form-control" id="inputDefault"
						maxlength="8" placeholder="비밀번호(최대 8자)" style="width: 250px;">
					</td>
				</tr>
				<tr>
					<td>
						<textarea name="cbComment" maxlength="500" placeholder="댓글을 입력해주세요"
	...
	<script>
		function content_Delete(){
			...
			var contentWriterID = "${cbContent.userID}";
			var contentPW = "${cbContent.contentPW}";
			
			if(contentWriterID){
				if(sessionUserID == contentWriterID){
					if(confirm("정말로 삭제하시겠습니까?")){
						location.href='ClubBoard_Delete.do?cNum=${cbContent.CBN_seq}&fileName=${cbContent.imgName}'
					}
				} else {
					alert("삭제할 수 없습니다");
				}
		...
		function content_Update(){
			...
			if(contentWriterID){
				if(sessionUserID == contentWriterID){
					location.href='ClubBoard_UpdateForm.do?cNum=${cbContent.CBN_seq}'
				} else {
					alert("수정할 수 없습니다");
				}
			} else if(!contentPW){
		...
		function commentDelete(commentNum, commentWriterID, commentWriterPW){
			...
			if(commentWriterID){
				if(sessionUserID == commentWriterID){
					if(confirm("정말로 삭제하시겠습니까?")){
						location.href='ClubBoard_Content_CommentDelete.do?cNum=${cbContent.CBN_seq}&commentNum='+commentNum;
					}
				} else {
					alert("삭제할 수 없습니다");
				}
		...
		function commentUpdate(commentWriterPW, commentNum){
			...
			var contentNum = "${cbContent.CBN_seq}";
			var comment = document.getElementById('comment'+commentNum).value;
			
			html += '<tr>';
			html += '<td style="text-align: left;">';
			html += '<form action="ClubBoard_Content_CommentUpdate.do">';
