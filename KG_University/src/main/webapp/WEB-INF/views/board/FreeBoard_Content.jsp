	...
	<div id="div_root">
		...
		<div id="section1">
			...
			<table border="0" width="90%">
				<tr>
					<td colspan="5" style="text-align: left;">
						<font size="4" style="font-weight: bold;">
							<c:choose>
								<c:when test="${fbContent.type eq 'university'}">
									[대학] ${fbContent.title}
								</c:when>
								<c:when test="${fbContent.type eq 'daily'}">
									[일상] ${fbContent.title}
								</c:when>
								<c:when test="${fbContent.type eq 'discuss'}">
									[토론] ${fbContent.title}
								</c:when>
								<c:otherwise>
									[일반] ${fbContent.title}
				...
				<tr style="text-align: left;">
					...
					<td width="400" style="text-align: right; padding-right: 20px;">
						<font style="font-weight: bold;">등록일</font> ${fbContent.regDate}
						<c:if test="${not empty fbContent.modifyDate}">
							&nbsp;&nbsp;<font style="font-weight: bold;">수정일</font> ${fbContent.modifyDate}
						</c:if>
					</td>
					<td width="80"><font style="font-weight: bold;">조회수</font> ${fbContent.hit}</td>
					<td width="150"><font style="font-weight: bold;">글쓴이</font> ${fbContent.userNickname}</td>
			...
			<div style="width: 90%; text-align: left; padding-left: 5%;">
				<c:if test="${not empty fbContent.imgName}">
					<img src ="<spring:url value='/resources/boardImg/${fbContent.imgName}'/>"><br>
					<br><br>
				</c:if>
				<pre><c:out value="${fbContent.content}" /></pre>
			</div>
			<br><br>
			...
			<table class="table table-hover">
				<c:forEach var="fbComment" items="${fbComment}" begin="${(boardCommentPagingVO.pageNum-1)*boardCommentPagingVO.perPageCnt}"
					...
					<tr>
						<td width="150" height="30" style="text-align: left;"><c:out value="${fbComment.userNickname}" /></td>
						<td style="text-align: left;"><pre><c:out value="${fbComment.boardComment}" /></pre></td>
						...
						<td width="10" style="text-align: right;">
							...
							onclick="commentUpdateCheck('${fbComment.boardComment}', '${fbComment.FBC_seq}', '${fbComment.userID}', '${fbComment.commentPW}')">수정</button> --%>
							<button id="${'comment'+=fbComment.FBC_seq}" value="${fbComment.boardComment}"
							class="badge badge-secondary" style="font-size: 11pt; padding: 3px;"
							onclick="commentUpdateCheck('${fbComment.FBC_seq}', '${fbComment.userID}', '${fbComment.commentPW}')">수정</button>
						</td>
						
						<td width="10" style="text-align: right;">
							<button type="button" class="badge badge-danger" style="font-size: 11pt; padding: 3px 6px;"
							onclick="commentDelete('${fbComment.FBC_seq}', '${fbComment.userID}', '${fbComment.commentPW}')">X</button>
							<%-- <button type="button" class="commentDelete" data-cNum="${fbComment.commentSeq}" >X</button>
							<script>
								$(".commentDelete").click(function(){
									self.location = "FreeBoard_Content_CommentDelete.do?"
									+ "cNum=${fbContent.FBN_seq}"
									+ "&commentNum=" + $(this).attr("data-cNum");
								});
							</script> --%>
						</td>
					</tr>
...
<!-- 세션확인, 닉네임도필요 -->
			...
			<table border="0" width="90%">
				<tr>
					<td height="40" style="text-align: left;">
						<input type="password" name="fbCommentPW" class="form-control" id="inputDefault"
						maxlength="8" placeholder="비밀번호(최대 8자)" style="width: 250px;">
					</td>
				</tr>
				<tr>
					<td>
						<textarea name="fbComment" maxlength="500" placeholder="댓글을 입력해주세요"
	...
	<script>
		function content_Delete(){
			...
			var contentPW = "${fbContent.contentPW}";
			
			if(contentWriterID){
				if(sessionUserID == contentWriterID){
					if(confirm("정말로 삭제하시겠습니까?")){
						location.href='FreeBoard_Delete.do?cNum=${fbContent.FBN_seq}&fileName=${fbContent.imgName}'
					}
				} else {
					alert("삭제할 수 없습니다");
				}
		...
		function content_Update(){
			...
			if(contentWriterID){
				if(sessionUserID == contentWriterID){
					location.href='FreeBoard_UpdateForm.do?cNum=${fbContent.FBN_seq}'
				} else {
					alert("수정할 수 없습니다");
				}
			} else if(!contentPW){
		...
		function commentDelete(commentNum, commentWriterID, commentWriterPW){
			...
			if(commentWriterID){
				if(sessionUserID == commentWriterID){
					if(confirm("정말로 삭제하시겠습니까?")){
						location.href='FreeBoard_Content_CommentDelete.do?cNum=${fbContent.FBN_seq}&commentNum='+commentNum;
					}
				} else {
					alert("삭제할 수 없습니다");
				}
		...
		function commentUpdate(commentWriterPW, commentNum){
			...
			var contentNum = "${fbContent.FBN_seq}";
			var comment = document.getElementById('comment'+commentNum).value;
			
			html += '<tr>';
			html += '<td style="text-align: left;">';
			html += '<form action="FreeBoard_Content_CommentUpdate.do">';
