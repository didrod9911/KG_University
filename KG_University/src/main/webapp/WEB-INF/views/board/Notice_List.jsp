...
<meta charset="UTF-8">
<title>KG_University - 공지사항</title>
	...
	<div id="div_root">
		...
		<div id="section1">
			...
			<table class="table table-hover">
				...
				<c:forEach var="noticeList" items="${noticeList}"	begin="${(boardPagingVO.pageNum-1)*boardPagingVO.perPageCnt}"
															end="${boardPagingVO.pageNum*boardPagingVO.perPageCnt-1}">
					<tr>
						<td width="70" height="25"><c:out value="${noticeList.NTN_seq}" /></td>
						<td width="100">
							<c:choose>
								<c:when test="${noticeList.type eq 'haksa'}">
									<c:out value="학사" />
								</c:when>
								<c:when test="${noticeList.type eq 'hang'}">
									<c:out value="행정" />
								</c:when>
								<c:when test="${noticeList.type eq 'janghak'}">
									<c:out value="장학" />
								</c:when>
								<c:when test="${noticeList.type eq 'hak'}">
									<c:out value="학과" />
								</c:when>
								<c:otherwise>
									<c:out value="기타" />
						...
						<td style="text-align: left; padding-left: 5px;">
							<a href="Notice_Content.do?cNum=${noticeList.NTN_seq}">
								<c:out value="${noticeList.title}" />
							</a>
							<c:if test="${noticeList.commentCnt > 0}">
								[<c:out value="${noticeList.commentCnt}" />]
							</c:if>
						</td>
						<td width="120"><c:out value="${noticeList.userNickname}" /></td>
						<td width="150"><c:out value="${noticeList.regDate}" /></td>
						<td width="80"><c:out value="${noticeList.hit}" /></td>
					</tr>
				</c:forEach>
			</table>
			</div>
			...
			<c:if test="${boardPagingVO.prev}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="Notice_List.do?page=${(boardPagingVO.pageBlock-2) * boardPagingVO.displayPageNum + 1}
							&searchType=${boardPagingVO.searchType}
			...
			<c:forEach var="page" begin="${boardPagingVO.startPage}" end="${boardPagingVO.endPage}">
				<c:choose>
					<c:when test="${boardPagingVO.pageNum eq page}">
						<font style="font-size: 13pt; font-weight: bold;">
							<a href="Notice_List.do?page=${page}
									&searchType=${boardPagingVO.searchType}
			...
			<c:if test="${boardPagingVO.next}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="Notice_List.do?page=${boardPagingVO.pageBlock * boardPagingVO.displayPageNum + 1}
							&searchType=${boardPagingVO.searchType}
			...
			<c:if test="${boardPagingVO.pageNum < boardPagingVO.lastPage}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="Notice_List.do?page=${boardPagingVO.lastPage}
							&searchType=${boardPagingVO.searchType}
