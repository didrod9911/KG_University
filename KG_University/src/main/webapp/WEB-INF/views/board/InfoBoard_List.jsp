...
<meta charset="UTF-8">
<title>KG_University - 정보게시판</title>
	...
	<div id="div_root">
		...
		<div id="section1">
				...
				<tr>
					...
					<td style="text-align: right;">
						<button type="button" class="btn btn-primary"
								onclick="location.href='InfoBoard_Write.do'"
			...
			<table class="table table-hover">
				...
				<c:forEach var="ibList" items="${ibList}"	begin="${(boardPagingVO.pageNum-1)*boardPagingVO.perPageCnt}"
															end="${boardPagingVO.pageNum*boardPagingVO.perPageCnt-1}">
					<tr>
						<td width="70" height="25"><c:out value="${ibList.IBN_seq}" /></td>
						<td width="100">
							<c:choose>
								<c:when test="${ibList.type eq 'haksa'}">
									<c:out value="학사" />
								</c:when>
								<c:when test="${ibList.type eq 'chi'}">
									<c:out value="취업" />
								</c:when>
								<c:when test="${ibList.type eq 'jil'}">
									<c:out value="질문" />
								</c:when>
								<c:otherwise>
									<c:out value="기타" />
						...
						<td style="text-align: left; padding-left: 5px;">
							<a href="InfoBoard_Content.do?cNum=${ibList.IBN_seq}">
								<c:out value="${ibList.title}" />
							</a>
							<c:if test="${ibList.commentCnt > 0}">
								[<c:out value="${ibList.commentCnt}" />]
							</c:if>
						</td>
						<td width="120"><c:out value="${ibList.userNickname}" /></td>
						<td width="150"><c:out value="${ibList.regDate}" /></td>
						<td width="70"><c:out value="${ibList.hit}" /></td>
					</tr>
				</c:forEach>
			</table>
			</div>
			...
			<c:if test="${boardPagingVO.pageNum ne 1}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="InfoBoard_List.do?page=1
			...
			<c:if test="${boardPagingVO.prev}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="InfoBoard_List.do?page=${(boardPagingVO.pageBlock-2) * boardPagingVO.displayPageNum + 1}
							&searchType=${boardPagingVO.searchType}
			...
			<c:forEach var="page" begin="${boardPagingVO.startPage}" end="${boardPagingVO.endPage}">
				<c:choose>
					<c:when test="${boardPagingVO.pageNum eq page}">
						<font style="font-size: 13pt; font-weight: bold;">
							<a href="InfoBoard_List.do?page=${page}
									&searchType=${boardPagingVO.searchType}
			...
			<c:if test="${boardPagingVO.next}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="InfoBoard_List.do?page=${boardPagingVO.pageBlock * boardPagingVO.displayPageNum + 1}
							&searchType=${boardPagingVO.searchType}
			...
			<c:if test="${boardPagingVO.pageNum < boardPagingVO.lastPage}">
				<font style="font-size: 13pt; font-weight: bold;">
					<a href="InfoBoard_List.do?page=${boardPagingVO.lastPage}
							&searchType=${boardPagingVO.searchType}
