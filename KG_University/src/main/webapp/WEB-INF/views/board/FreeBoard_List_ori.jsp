				...
				<tr>
					...
					<td style="text-align: right;">
						<button type="button" class="btn btn-primary"
								onclick="location.href='FreeBoard_Write.do'"
			...
			<table class="table table-hover">
				...
				<c:forEach var="fbList" items="${fbList}"	begin="${(boardPagingVO.pageNum-1)*boardPagingVO.perPageCnt}"
					...
					<tr>
						...
						<td width="100">
							<c:choose>
								<c:when test="${fbList.type eq 'university'}">
									<c:out value="대학" />
								</c:when>
								<c:when test="${fbList.type eq 'daily'}">
									<c:out value="일상" />
								</c:when>
								<c:when test="${fbList.type eq 'discuss'}">
									<c:out value="토론" />
								</c:when>
								<c:otherwise>
									<c:out value="일반" />
						...
						<td width="120"><c:out value="${fbList.userNickname}" /></td>
						<td width="150"><c:out value="${fbList.regDate}" /></td>
						<td width="80"><c:out value="${fbList.hit}" /></td>
					</tr>
				</c:forEach>
			</table>
			</div>
